...
export default function HeroVideoDialog({
  ...
  useEffect(() => {
    if (isVideoOpen && videoRef.current && !isPausedManually) {
      videoRef.current.play();
    }
  }, [isVideoOpen, isPausedManually]);
      ...
      <div className="group relative cursor-pointer" onClick={() => setIsVideoOpen(true)}>
        ...
        <div className="absolute inset-0 flex scale-[0.9] items-center justify-center rounded-2xl transition-all duration-200 ease-out group-hover:scale-100">
          <div className="flex size-28 items-center justify-center rounded-full bg-primary/10 backdrop-blur-md">
            <div className="relative flex size-20 scale-100 items-center justify-center rounded-full bg-gradient-to-b from-[#C7397C] to-[#73319F] transition-all duration-200 ease-out group-hover:scale-[1.2]">
              <Play
      ...
      <AnimatePresence>
        {isVideoOpen && (
          <motion.div
            ...
            onClick={() => setIsVideoOpen(false)} // Close modal when clicking outside
          >
            <motion.div
              {...selectedAnimation}
